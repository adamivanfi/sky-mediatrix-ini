 ...
 * @author vogtda
 */
public class MockNewDbStaticServiceProvider extends Thread {
	...
	public static void main(String args[]) throws Exception {
		if (args.length > 0)
			MockNewDbStaticServiceProvider.address = args[0];
		if (args.length > 1)
			MockNewDbStaticServiceProvider.setupScript = args[1];
		if (args.length > 2)
			MockNewDbStaticServiceProvider.dataScript = args[2];

		new MockNewDbStaticServiceProvider().start();
	}

	public void run() {
		SkyLogger.getMediatrixLogger().info("Starting Provider at " + address);
		implementor = new MockNewDBStaticPortTypeImpl();
