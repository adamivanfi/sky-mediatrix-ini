...
String recipients="gregor.meinusch@nttdata.com"; // nicht im Urlaub :-) Heino.Kappher@nttdata.com
...
dbi="Mediatrix_Admin"
mxdba = Sql.newInstance('jdbc:oracle:thin:@s-ng-pdmsx3-b.premiere.de:1521:MEXPRODN','MEIN49', 'gmeNTTgme94', 'oracle.jdbc.driver.OracleDriver') //ggf. oracle.jdbc.OracleDriver
mxdba.withStatement { 
   stmt -> stmt.queryTimeout = 60 
} 
...
mxdba.eachRow(sql) { row ->
       ...
       alerts+="${row.statusdate} \t ${dbi}: ${row.checkname} MXDB \t ${row.currentstatus} \t ${row.description} \n\r \n\r"
}

dbi="Contex_Admin"
cxdba = Sql.newInstance('jdbc:oracle:thin:@s-ng-pdmsx3-b.premiere.de:1521:CONPRODN','MEIN49', 'gmeNTTgme94', 'oracle.jdbc.driver.OracleDriver') //ggf. oracle.jdbc.OracleDriver
cxdba.withStatement { 
   stmt -> stmt.queryTimeout = 60 
}
cxdba.eachRow(sql) { row ->
       if (row.currentstatus == '3_ESCALACTION'){
            critical=true;
            escalations++
       } else  if (row.currentstatus == '2_ERROR'){
...
" where (sysdate - interval '16' MINUTE) < TIME "+
" and count_wait                         >100 "+
" and parameter not like '500_CRM_Activity' and parameter not like '550%' and parameter not like '220%' and parameter not like '810%' and parameter not like 'SL3K_%' and parameter not like 'Move_%' and parameter not like 'Archiv' "+
" group by parameter "+
"  "+
" union  "+
"  "+
...
"   CASE "+
"     when abs(ctx_manuelle_indizierung.counter - mx_manuelle_indizierung.counter) > 100 "+
"     then '3_ESCALACTION' "+
"     when abs(ctx_manuelle_indizierung.counter - mx_manuelle_indizierung.counter) > 50 "+
"     then '2_ERROR' "+
"     when abs(ctx_manuelle_indizierung.counter - mx_manuelle_indizierung.counter) > 40 "+
"     then '1_WARNING' "+
"     else '0_OK' "+
"   END currentstatus "+
" from "+
...
cxdb.eachRow(sql) { row ->
       if (row.currentstatus == '3_ESCALACTION'){
            critical=true;
            escalations++
            alerts+="${row.statusdate} \t ${dbi}: ${row.checkname} CXDB \t ${row.currentstatus} \t ${row.description} \t ${row.anzahl} \n\r \n\r"
       } else  if (row.currentstatus == '2_ERROR'){
            errors++
            alerts+="${row.statusdate} \t ${dbi}: ${row.checkname} CXDB \t ${row.currentstatus} \t ${row.description} \t ${row.anzahl} \n\r \n\r"
       } else if (row.currentstatus == '1_WARNING'){
...
"   select sysdate statusdate,  "+
           ...
           "CASE   "+
             ...
             "else '0_OK'   "+
           "END currentstatus   "+
    " from (select e.subject,e.sender, count(*) from ityx_mx.frage f, ityx_mx.email e where f.emailid = e.id and f.teilprojektid=110 and f.status not in ('erledigt','blockiert') and f.geloeschtam=0 and subject is not null group by e.subject,e.sender having count(*) > 1) " +
...
mxdb.eachRow(sql) { row ->
     ...
     critical=true;
     escalations++;
     alerts+="Critical Problem with DB connection: "+ dbi ;
     alerts+="\n\r \n\r"+e.message
}
...
//     session = Session.getDefaultInstance(properties, null);
     ...
     //message.setRecipient(Message.RecipientType.TO, new InternetAddress("gregor.meinusch@nttdata.com"));
     message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(recipients));
     
     message.setSubject(subject);
     //mailtextstring = new String(alerts)             
     //message.setDataHandler(new DataHandler(new ByteArrayDataSource(mailtextstring, "text/text")));
...
//} else {
//    println "no mail needed: "+subject
//    println sql
//    println alerts
}

} finally {
    if (mxdba) mxdba.connection.close()  
    if (cxdba) cxdba.connection.close()  
    if (cxdb)  cxdb.connection.close()  
    if (mxdb)  cxdb.connection.close()  
