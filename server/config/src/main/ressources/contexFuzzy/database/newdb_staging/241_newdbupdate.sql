...
select count(*) into v_tmpcounter from CUSTOMER@NEWDB_PROD.SKY.DE ;
dbms_output.put_line('NEWDB CUSTOMER-Actions    Items: ' ||  v_tmpcounter ||'  '|| sysdate);
select count(*) into v_tmpcounter from CONTRACT@NEWDB_PROD.SKY.DE ;
dbms_output.put_line('NEWDB CONTRACT-Actions    Items: ' ||  v_tmpcounter ||'  '|| sysdate);
select count(*) into v_tmpcounter from ASSET@NEWDB_PROD.SKY.DE ;
dbms_output.put_line('NEWDB ASSET-Actions       Items: ' ||  v_tmpcounter ||'  '|| sysdate);
select count(*) into v_tmpcounter from CAMPAIGN@NEWDB_PROD.SKY.DE ;
dbms_output.put_line('NEWDB CAMPAIGN-Actions     Items: ' ||  v_tmpcounter ||'  '|| sysdate);
select count(*) into v_tmpcounter from CAMPAIGN_CONF@NEWDB_PROD.SKY.DE ;
...
-- stage CAMPAIGN_CONF
 dbms_output.put_line('NewDB Import    STAGE CAMPAIGN_CONF # ' || sysdate); 
  v_i  := 0;
 for rec in (SELECT CAMPAIGN_ID, CAMPAIGN_TYPE, STATUS, START_DATE, END_DATE, OPERATION, OPERATION_DATE  FROM CAMPAIGN_CONF@NEWDB_PROD.SKY.DE ) loop  
...
-- stage CAMPAIGN
 ...
 v_i  := 0;
 for rec in (SELECT CUSTOMER_ID, CAMPAIGN_ID, OPERATION,OPERATION_DATE  FROM CAMPAIGN@NEWDB_PROD.SKY.DE ) loop  
...
-- stage asset
 ...
 v_i  := 0;
 for rec in (SELECT CUSTOMER_ID, CONTRACT_ID, SERIAL_NUMBER,  STATUS, OPERATION, OPERATION_DATE FROM asset@NEWDB_PROD.SKY.DE ) loop  
...
-- stage contract
 ...
 v_i  := 0;
  for rec in (SELECT CONTRACT_ID, CUSTOMER_ID, HOUSE_NUMBER,  STREET, ZIPCODE, CITY,COUNTRY,ACCOUNT_NUMBER,BANK_CODE, BANK_ACCOUNT_HOLDER, STATUS, OPERATION, OPERATION_DATE, WM_FLG, RECEPTION, OPERATOR, PLATFORM FROM CONTRACT@NEWDB_PROD.SKY.DE ) loop  
...
-- stage customer
 ...
 v_i  := 0;
 for rec in (SELECT ROW_ID, CUSTOMER_ID, MOBILE_NUMBER, EMAIL_ADDRESS,  FIRST_NAME, LAST_NAME, TELEPHONE_NUMBER, STATUS, OPERATION, OPERATION_DATE   FROM customer@NEWDB_PROD.SKY.DE ) loop  
...
-- stage CAMPAIGN_CONF
 dbms_output.put_line('NewDB Import2 ' || v_updatetime || '   STAGE CAMPAIGN_CONF # ' || sysdate); 
  v_i  := 0;
 for rec in (SELECT CAMPAIGN_ID, CAMPAIGN_TYPE, STATUS, START_DATE, END_DATE, OPERATION, OPERATION_DATE FROM CAMPAIGN_CONF@NEWDB_PROD.SKY.DE WHERE OPERATION_DATE > v_updatetime) loop  
 INSERT INTO NEWDB_CAMPAIGN_CONF_STAGE(CAMPAIGN_ID, CAMPAIGN_TYPE, STATUS, START_DATE, END_DATE, OPERATION, OPERATION_DATE) VALUES
 (rec.CAMPAIGN_ID, rec.CAMPAIGN_TYPE, rec.STATUS, rec.START_DATE, rec.END_DATE, rec.OPERATION, rec.OPERATION_DATE);
  v_i  := v_i  + 1;
  if MOD(v_i, c_commitsize)=0 THEN
...
-- stage CAMPAIGN
 ...
 v_i  := 0;
 for rec in (SELECT CUSTOMER_ID, CAMPAIGN_ID, OPERATION,OPERATION_DATE  FROM CAMPAIGN@NEWDB_PROD.SKY.DE WHERE OPERATION_DATE > v_updatetime) loop  
 INSERT INTO NEWDB_CAMPAIGN_STAGE(CUSTOMER_ID, CAMPAIGN_ID, OPERATION,OPERATION_DATE) VALUES
 (rec.CUSTOMER_ID, rec.CAMPAIGN_ID, rec.OPERATION,rec.OPERATION_DATE);
  v_i  := v_i  + 1;
  if MOD(v_i, c_commitsize)=0 THEN
...
-- stage asset
 dbms_output.put_line('NewDB Import    STAGE ASSET # ' || sysdate); 
 v_i  := 0;
 for rec in (SELECT CUSTOMER_ID, CONTRACT_ID, SERIAL_NUMBER,  STATUS, OPERATION, OPERATION_DATE FROM asset@NEWDB_PROD.SKY.DE WHERE OPERATION_DATE > v_updatetime) loop  
 INSERT INTO NEWDB_ASSET_STAGE(CUSTOMER_ID, CONTRACT_ID, SERIAL_NUMBER,  STATUS, OPERATION, OPERATION_DATE) VALUES
(rec.CUSTOMER_ID, rec.CONTRACT_ID, rec.SERIAL_NUMBER,  rec.STATUS, rec.OPERATION, rec.OPERATION_DATE);
  v_i  := v_i  + 1;
  if MOD(v_i, c_commitsize)=0 THEN
...
-- stage contract
 ...
 v_i  := 0;
  for rec in (SELECT CONTRACT_ID, CUSTOMER_ID, HOUSE_NUMBER,  STREET, ZIPCODE, CITY,COUNTRY,ACCOUNT_NUMBER,BANK_CODE, BANK_ACCOUNT_HOLDER, STATUS, OPERATION, OPERATION_DATE, WM_FLG, RECEPTION, OPERATOR, PLATFORM FROM CONTRACT@NEWDB_PROD.SKY.DE WHERE OPERATION_DATE > v_updatetime ) loop  
 INSERT INTO NEWDB_CONTRACT_STAGE(CONTRACT_ID, CUSTOMER_ID, HOUSE_NUMBER,  STREET, ZIPCODE, CITY,COUNTRY,ACCOUNT_NUMBER,BANK_CODE, BANK_ACCOUNT_HOLDER, STATUS, OPERATION, OPERATION_DATE, WM_FLG, RECEPTION, OPERATOR, PLATFORM) VALUES
(rec.CONTRACT_ID, rec.CUSTOMER_ID, rec.HOUSE_NUMBER,  rec.STREET, rec.ZIPCODE, rec.CITY,rec.COUNTRY,rec.ACCOUNT_NUMBER,rec.BANK_CODE, rec.BANK_ACCOUNT_HOLDER, rec.STATUS, rec.OPERATION, rec.OPERATION_DATE, rec.WM_FLG, rec.RECEPTION, rec.OPERATOR, rec.PLATFORM);
 v_i  := v_i  + 1;
  if MOD(v_i, c_commitsize)=0 THEN
...
-- stage customer
 ...
 v_i  := 0;
 for rec in (SELECT ROW_ID, CUSTOMER_ID, MOBILE_NUMBER, EMAIL_ADDRESS,  FIRST_NAME, LAST_NAME, TELEPHONE_NUMBER, STATUS, OPERATION, OPERATION_DATE FROM customer@NEWDB_PROD.SKY.DE WHERE OPERATION_DATE > v_updatetime) loop  
 INSERT INTO NEWDB_CUSTOMER_STAGE(ROW_ID, CUSTOMER_ID, MOBILE_NUMBER, EMAIL_ADDRESS, FIRST_NAME, LAST_NAME, TELEPHONE_NUMBER, STATUS, OPERATION, OPERATION_DATE) VALUES
(rec.ROW_ID, rec.CUSTOMER_ID, rec.MOBILE_NUMBER, rec.EMAIL_ADDRESS,  rec.FIRST_NAME, rec.LAST_NAME, rec.TELEPHONE_NUMBER, rec.STATUS, rec.OPERATION, rec.OPERATION_DATE);
 v_i  := v_i  + 1;
  if MOD(v_i, c_commitsize)=0 THEN
...
-- set new TS
NEWDB.DMS_STOP_PROC@NEWDB_PROD.SKY.DE(v_updatetime);
