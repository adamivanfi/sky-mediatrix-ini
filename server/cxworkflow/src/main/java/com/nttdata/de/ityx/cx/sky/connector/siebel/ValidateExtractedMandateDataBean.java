 ...
 * 
 */
public class ValidateExtractedMandateDataBean  extends AbstractWflBean  {
	...
	public void execute(IFlowObject flowObject) {
		...
		if (customerData != null && customerData.size() > 0) {
			...
			verified = verified && (mandateNumber != null && mandateNumber.trim().length() > 0 && !"0".equals(mandateNumber.trim()));
			if (verified) {
