 ...
 * @author VOGT41
 */
public class EnrichDocumentBean extends AbstractWflBean {
	...
	private void writeCustomerToFlow(IFlowObject flowObject, IndexingCustomer customer, String isActive) {
		...
		if (!noDoc) {
			...
			docId = DocContainerUtils.getDocID(document);
		}
		...
		addToListIfNotNull(list, TagMatchDefinitions.CUSTOMER_ADDITIONAL_ADDRESS, customer.getAdditionalAddress());  // SIT-17-05-099
	...
	private String enrichFromContractTable(IndexingCustomer customer) throws Exception {
		...
		try {
			...
			String sqlQuery = "select CONTRACT_ID, CUSTOMER_ID, HOUSE_NUMBER,  STREET, ZIPCODE, CITY,COUNTRY,ACCOUNT_NUMBER,BANK_CODE, BANK_ACCOUNT_HOLDER, STATUS,CUSTOMER_BIC,MANDATE_STATUS,MANDATE_REF_ID, OPERATOR, PLATFORM, RECEPTION, OPERATION_DATE, WM_FLG, CONTRACT_TYPE, RATECARD_FLG, CNTRSTARTDATE  " + "from " + tableContract + " where customer_id = '" + customerNumber + "' and contract_id = '" + contractNumber + "' ";
			if ((customerNumber == null || customerNumber.equals("") || customerNumber.equals("0")) && (contractNumber != null && !contractNumber.equals("") && !contractNumber.equals("0"))) {
				sqlQuery = "select CONTRACT_ID, CUSTOMER_ID, HOUSE_NUMBER,  STREET, ZIPCODE, CITY,COUNTRY,ACCOUNT_NUMBER,BANK_CODE, BANK_ACCOUNT_HOLDER, STATUS,CUSTOMER_BIC,MANDATE_STATUS,MANDATE_REF_ID, OPERATOR, PLATFORM, RECEPTION, OPERATION_DATE, WM_FLG, CONTRACT_TYPE, RATECARD_FLG, CNTRSTARTDATE  " + "from " + tableContract + " where  contract_id = '" + contractNumber + "'";
				...
				sqlQuery = "select CONTRACT_ID, CUSTOMER_ID, HOUSE_NUMBER,  STREET, ZIPCODE, CITY,COUNTRY,ACCOUNT_NUMBER,BANK_CODE, BANK_ACCOUNT_HOLDER, STATUS,CUSTOMER_BIC,MANDATE_STATUS,MANDATE_REF_ID,  OPERATOR, PLATFORM, RECEPTION, OPERATION_DATE, WM_FLG, CONTRACT_TYPE, RATECARD_FLG, CNTRSTARTDATE  " + "from " + tableContract + " where  customer_id = '" + customerNumber + "'  order by OPERATION_DATE ";
			} else if ((customerNumber == null || customerNumber.equals("") || customerNumber.equals("0")) && (contractNumber == null || contractNumber.equals("") || contractNumber.equals("0"))) {
	...
	private void enrichFromAssetTable(IndexingCustomer customer) {
		...
		try {
			...
			String sqlQuery = "SELECT nc.CUSTOMER_ID, nc.CONTRACT_ID, nc.OPERATOR, na.SERIAL_NUMBER, na.STATUS, na.OPERATION_DATE FROM " + tableAsset + " na, " + tableContract + " nc " + "WHERE na.CUSTOMER_ID = '" + customer_id + "' AND na.CONTRACT_ID = nc.CONTRACT_ID AND na.CUSTOMER_ID = nc.CUSTOMER_ID AND na.SERIAL_NUMBER = '" + smartcardNumber + "' ORDER BY na.STATUS, na.OPERATION_DATE DESC";
			// Tries to enrich contract.
			if (!missingSmartCard && missingCustomer) {
				sqlQuery = "SELECT nc.CUSTOMER_ID, nc.CONTRACT_ID, nc.OPERATOR, na.SERIAL_NUMBER, na.STATUS, na.OPERATION_DATE FROM " + tableAsset + " na, " + tableContract + " nc " + "WHERE na.CONTRACT_ID = nc.CONTRACT_ID AND na.CUSTOMER_ID = nc.CUSTOMER_ID AND na.SERIAL_NUMBER = '" + smartcardNumber + "' ORDER BY nc.CUSTOMER_ID, nc.CONTRACT_ID, na.STATUS, na.OPERATION_DATE DESC";
				// Tries to enrich contract and SMC.
			} else if (!missingCustomer && missingSmartCard) {
				sqlQuery = "SELECT nc.CUSTOMER_ID, nc.CONTRACT_ID, nc.OPERATOR, na.SERIAL_NUMBER, na.STATUS, na.OPERATION_DATE FROM " + tableAsset + " na, " + tableContract + " nc " + "WHERE na.CUSTOMER_ID = '" + customer_id + "' AND na.CONTRACT_ID = nc.CONTRACT_ID AND na.CUSTOMER_ID = nc.CUSTOMER_ID ORDER BY na.STATUS, na.OPERATION_DATE DESC";
				// Tries to enrich SMC.
			} else if (missingCustomer && missingSmartCard && !missingContract) {
				sqlQuery = "SELECT nc.CUSTOMER_ID, nc.CONTRACT_ID, nc.OPERATOR, na.SERIAL_NUMBER, na.STATUS, na.OPERATION_DATE FROM " + tableAsset + " na, " + tableContract + " nc " + "WHERE na.CONTRACT_ID = '" + contractNumber + "' AND na.CONTRACT_ID = nc.CONTRACT_ID AND na.CUSTOMER_ID = nc.CUSTOMER_ID ORDER BY na.STATUS, na.OPERATION_DATE DESC";
			}
	...
	private void enrichFromCampaignTable(IndexingCustomer customer) {
		...
		try {
			...
			String sqlQuery = "select campaign_type from newdb_campaign_conf ccf where end_date > sysdate and start_date < sysdate and status='AKTIV' and exists (" + "select customer_id from newdb_campaign ca where ca.campaign_id=ccf.campaign_id and ca.customer_id='" + customer.getNumber() + "' ) group by campaign_type ";
			...
			try {
				...
				while (campaign.next()) {
					...
					if (campaignColumn != null && !campaignColumn.equalsIgnoreCase("null") && !campaignColumn.isEmpty()) {
	...
	private void enrichFromCustomerTable(IndexingCustomer customer) throws Exception {
		...
		ResultSet rs = null;
		try {
			String sqlQuery = "select ROW_ID, CUSTOMER_ID, MOBILE_NUMBER, EMAIL_ADDRESS,  FIRST_NAME, LAST_NAME, TELEPHONE_NUMBER, STATUS, OPERATION_DATE from " + tableCustomer + " where customer_id = '" + customer.getNumber() + "' ORDER BY OPERATION_DATE DESC";
	...
	public void execute(IFlowObject flowObject) throws Exception {
		...
		SkyLogger.getItyxLogger().debug(verifiedCustomerNumber + " Enrich document " + ctx_docid + " for customer with customer number " + verifiedCustomerNumber + ", contract number " + verifiedContractNumber + " and smartcard number " + verifiedSmartcardNumber);
