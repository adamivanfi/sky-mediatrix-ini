...
package de.sky.integration.web.customerdataservice._1_0;
...
@javax.jws.WebService(serviceName = "CustomerDataService",
		...
		wsdlLocation = "wsdl/IF32_NewDB/CustomerDataService.wsdl",
		endpointInterface = "de.sky.integration.web.customerdataservice._1_0.PortType")
public class PortTypeImpl implements PortType {
	...
	public PortTypeImpl(String setupScript, String dataScript) {
		setupTestDatabase(setupScript, dataScript);
	}

	public void cleanupDatabase(final String pCleanupScript) {
		SkyLogger.getTestLogger().info("Cleanup database - dropping tables.");
		if (pCleanupScript != null) {
			File cleanupScriptFile = new File(pCleanupScript);
			if (cleanupScriptFile.exists()) {
				try {
					SkyLogger.getTestLogger().debug("Using file: " + cleanupScriptFile.getAbsolutePath());
					executeSQLScript(new FileInputStream(cleanupScriptFile));
				} catch (FileNotFoundException e) {
					SkyLogger.getTestLogger().debug("File " + cleanupScriptFile.getAbsolutePath() + " not found. Using packaged structure.", e);
	...
	private void setupTestDatabase(String setupScript, String dataScript) {
		try {
			...
			SkyLogger.getTestLogger().error("Unable to establish memory database.", e1);
			return;
		}

		SkyLogger.getTestLogger().info("Setup database - creating tables.");
		if (setupScript != null) {
			File setupScriptFile = new File(setupScript);
			if (setupScriptFile.exists()) {
				try {
					SkyLogger.getTestLogger().debug("Using file: " + setupScriptFile.getAbsolutePath());
					executeSQLScript(new FileInputStream(setupScriptFile));
				} catch (FileNotFoundException e) {
					SkyLogger.getTestLogger().debug("File " + setupScriptFile.getAbsolutePath() + " not found. Using packaged structure.", e);
		...
		if (dataScript != null) {
			...
			if (file.exists()) {
				try {
					SkyLogger.getTestLogger().debug("Using file: " + file.getAbsolutePath());
					executeSQLScript(new FileInputStream(file));
				} catch (FileNotFoundException e) {
					SkyLogger.getTestLogger().warn("File " + file.getAbsolutePath() + " not found. Using packaged data.", e);
		...
		try {
			...
			SkyLogger.getTestLogger().debug("MEMDB imported " + customers + " customers");
		} catch (SQLException e) {
			SkyLogger.getTestLogger().error("Failed to query for customers.", e);
		}
	...
	public GetDynamicCustomerDataResponseType getDynamicCustomerData(GetDynamicCustomerDataRequestType req) {
		SkyLogger.getTestLogger().debug("Executing operation getDynamicCustomerData " + req);
		try {
			...
			SkyLogger.getTestLogger().debug(" query CustomerId " + req.getCustomerId());
			...
			try {
				...
				if (rs.next()) {
					...
					SkyLogger.getTestLogger().debug("set CustomerId " + rs.getString("CUSTOMER_ID"));
				...
				String sqlQuery = "SELECT CAMPAIGN_TYPE FROM NEWDB_CAMPAIGN_CONF CCF " + "WHERE END_DATE > SYSDATE AND START_DATE < SYSDATE AND STATUS='AKTIV' AND " + "EXISTS (SELECT CUSTOMER_ID " + "FROM NEWDB_CAMPAIGN CA " + "WHERE CA.CAMPAIGN_ID=CCF.CAMPAIGN_ID AND " + "CA.CUSTOMER_ID='" + req.getCustomerId()/*
																												 * customer
																												 * .
																												 * getNumber
																												 * (
				...
				if (contractId != null && !"".equals(contractId)) {
					sql = "SELECT * FROM NEWDB_CONTRACT WHERE CUSTOMER_ID = '" + customer.getCustomerId() + "' AND CONTRACT_ID = '" + contractId + "'";
					withContract = true;
				} else {
					sql = "SELECT * FROM NEWDB_CONTRACT WHERE CUSTOMER_ID = '" + customer.getCustomerId() + "'";
				}
				 ...
				 * contract.
				 */
				SkyLogger.getTestLogger().debug(sql);
				while (rs.next()) {
					...
					SkyLogger.getTestLogger().debug(rs.getString("CONTRACT_ID"));
					/* *****************************************************************************
			...
			_return.setCustomerId(req.getCustomerId());
			SkyLogger.getTestLogger().debug(NewDBConnectorImpl.outputResponse(_return));
