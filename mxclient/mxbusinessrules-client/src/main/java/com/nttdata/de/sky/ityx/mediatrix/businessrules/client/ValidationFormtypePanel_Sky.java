...
import de.ityx.tools.ccbox.CCBox;
...
// FocusListener, ItemListener, ITagMatchView
public class ValidationFormtypePanel_Sky extends JPanel implements IFormtypeViewer, KeyListener, ActionListener, ListSelectionListener {
	...
	static {
		...
		ValidationFormtypePanel_Sky.tagNames = new LinkedHashMap<String, String>();
		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.CUSTOMER_ID, "CUSTOMER_ID");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.TAGMATCH_CONTRACT_NUMBER, "CONTRACT_ID");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.TAGMATCH_SMARTCARD_NUMBER, "SERIAL_NUMBER");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.SEPA_MANDATE_NUMBER, "MANDATE_REF_ID");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.CUSTOMER_FIRST_NAME, "FIRST_NAME");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.CUSTOMER_LAST_NAME, "LAST_NAME");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.CUSTOMER_STREET, "STREET");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.CUSTOMER_ZIP_CODE, "ZIPCODE");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.CUSTOMER_CITY, "CITY");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.TAGMATCH_EMAIL, "EMAIL_ADDRESS");

		ValidationFormtypePanel_Sky.tagNames.put(TagMatchDefinitions.TAGMATCH_BANK, "ACCOUNT_NUMBER");


		ValidationFormtypePanel_Sky.firstColumnTags = new TreeSet<String>();
		...
		ValidationFormtypePanel_Sky.firstColumnTags.add(TagMatchDefinitions.CUSTOMER_ID);
		ValidationFormtypePanel_Sky.firstColumnTags.add(TagMatchDefinitions.TAGMATCH_SMARTCARD_NUMBER);
		ValidationFormtypePanel_Sky.firstColumnTags.add(TagMatchDefinitions.CUSTOMER_FIRST_NAME);
		ValidationFormtypePanel_Sky.firstColumnTags.add(TagMatchDefinitions.CUSTOMER_STREET);
		ValidationFormtypePanel_Sky.firstColumnTags.add(TagMatchDefinitions.CUSTOMER_ZIP_CODE);
		ValidationFormtypePanel_Sky.firstColumnTags.add(TagMatchDefinitions.TAGMATCH_EMAIL);
		//ValidationFormtypePanel_Sky.firstColumnTags.add(TagMatchDefinitions.SEPA_MANDATE_NUMBER);

		ValidationFormtypePanel_Sky.tagNames4PrefixSearch = new TreeSet<String>();
		ValidationFormtypePanel_Sky.tagNames4PrefixSearch.add(TagMatchDefinitions.CUSTOMER_FIRST_NAME);
		ValidationFormtypePanel_Sky.tagNames4PrefixSearch.add(TagMatchDefinitions.CUSTOMER_LAST_NAME);
		ValidationFormtypePanel_Sky.tagNames4PrefixSearch.add(TagMatchDefinitions.CUSTOMER_STREET);
		ValidationFormtypePanel_Sky.tagNames4PrefixSearch.add(TagMatchDefinitions.CUSTOMER_CITY);
	...
	public class FuzzySearchAction extends AbstractAction {
		...
		public FuzzySearchAction() {
			...
			SkyLogger.getMediatrixLogger().debug("FuzzySearchAction: constructor");
			SkyLogger.getClientLogger().debug("FuzzySearchAction: constructor");
			SkyLogger.getCommonLogger().debug("FuzzySearchAction: constructor");
			SkyLogger.getItyxLogger().debug("FuzzySearchAction: constructor");
			System.out.println("FuzzySearchAction: constructor");
			System.err.println("FuzzySearchAction: constructor");
		...
		public void actionPerformed(ActionEvent e) {
			SkyLogger.getMediatrixLogger().debug("FuzzySearchAction: " + e.getSource());
			try {
				...
				if (!(parent instanceof ITagMatchViewElement)) {
					ValidationFormtypePanel_Sky.this.lbStatus.setVisible(true);
					ValidationFormtypePanel_Sky.this.lbStatus.setText(Messages.getString("ValidationFormtypePanel.1"));
					return;
				}

				executeFuzzySearch();
				ValidationFormtypePanel_Sky.lastFocusComponent = (JComponent) focusOwner;
			} finally {
		...
		private void executeFuzzySearch() {
			SkyLogger.getMediatrixLogger().debug("FuzzySearchAction: execute search ");
			final Integer projektId = ValidationFormtypePanel_Sky.this.question.getProjectId();
			List<Object> parameter = new ArrayList<Object>();
			parameter.add(projektId);
			Request request = new Request();
			for (String tagIdentifier : ValidationFormtypePanel_Sky.viewElements.keySet()) {
				TagMatchViewElement viewElement = ValidationFormtypePanel_Sky.viewElements.get(tagIdentifier);
				if (viewElement != null) {
					String textValue = viewElement.getTextValue();
					// Only searches for something.
					if (textValue != null && textValue.length() > 0) {
						int mode = IFuzzyMatcherEngine.CANDIDATES_FUZZY;
						if (ValidationFormtypePanel_Sky.tagNames4PrefixSearch.contains(tagIdentifier) && textValue.length() > 2) {
							mode = IFuzzyMatcherEngine.CANDIDATES_PREFIX;
						}
						SkyLogger.getMediatrixLogger().debug("FuzzySearchAction: text value: " + textValue);
						request.addColumn(ValidationFormtypePanel_Sky.tagNames.get(tagIdentifier), textValue, true, 1, mode);
					}
				}
			}

			parameter.add(request);
			SkyLogger.getMediatrixLogger().debug("FuzzySearchAction: request: " + request.toString());
			...
			SkyLogger.getMediatrixLogger().debug("FuzzySearchAction: search result: " + result.size());
		...
		public Response filterResults(Response input){
			...
			List<Row> standardCase=new LinkedList<>();
			...
			if (output.getRowCount()<maxRecords && standardCase.size()>0){
				for (Row row:standardCase) {
					if (i<maxRecords) {
						output.addRow(row);
					}
					i++;
		...
		private void setFuzzyResponseTable(Response result) {
			ValidationFormtypePanel_Sky.this.fuzzyResponse = result;
			setTableModel(true);
			if (ValidationFormtypePanel_Sky.this.fuzzyResponse.getRowCount() == 0) {
				ValidationFormtypePanel_Sky.this.lbStatus.setVisible(true);
				ValidationFormtypePanel_Sky.this.lbStatus.setText(Messages.getString("ValidationFormtypePanel.160"));
			} else if (ValidationFormtypePanel_Sky.this.fuzzyResponse.getRowCount() == 1) {
				ValidationFormtypePanel_Sky.this.lbStatus.setVisible(false);
			} else if (ValidationFormtypePanel_Sky.this.fuzzyResponse.getRowCount() > 1) {
				ValidationFormtypePanel_Sky.this.tbSearchResults.requestFocus();
				final int rowCount = ValidationFormtypePanel_Sky.this.tbSearchResults.getRowCount();
				if (rowCount > 0 && ValidationFormtypePanel_Sky.this.fuzzyResponse.getRow(0).getError() == 0.0 && (rowCount == 1 || ValidationFormtypePanel_Sky.this.fuzzyResponse.getRow(1).getError() > 0.0)) {
					ValidationFormtypePanel_Sky.this.tbSearchResults.getSelectionModel().setSelectionInterval(0, 0);
					ValidationFormtypePanel_Sky.this.tbSearchResults.getSelectionModel().setLeadSelectionIndex(0);
					ValidationFormtypePanel_Sky.this.tbSearchResults.getColumnModel().getSelectionModel().setLeadSelectionIndex(0);
					valueChanged(new ListSelectionEvent(ValidationFormtypePanel_Sky.this.tbSearchResults, 0, 0, true));
				}
				ValidationFormtypePanel_Sky.this.lbStatus.setVisible(false);
			}
		}
	...
	public class ExactMatchHighlighter implements TableCellRenderer {
		 ...
		 *
		 * @param defaultRenderer The delegate renderer.
		 */
	...
	public class FocusTableAction extends AbstractAction {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			if (!ValidationFormtypePanel_Sky.this.tbSearchResults.isFocusOwner()) {
				ValidationFormtypePanel_Sky.this.tbSearchResults.requestFocus();
				if (ValidationFormtypePanel_Sky.this.tbSearchResults.getRowCount() > 0) {
					ValidationFormtypePanel_Sky.this.tbSearchResults.getSelectionModel().setSelectionInterval(0, 0);
				}
			}
		}
	...
	public class SaveMeAction extends AbstractAction {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			final CDocument doc = ValidationFormtypePanel_Sky.this.document.getDocument(0);
			final String formtype = doc.getFormtype();
			if (!isEnabled() || formtype.equals("unclassified") || formtype.equals("systemdefault")|| formtype.equals("kuendigung_automatisch")) {
				JOptionPane.showMessageDialog(null, "Ungültiger Dokumenttyp");
				return;
			}
			...
			final TagMatch tagMatch = ValidationFormtypePanel_Sky.this.document.getTags().get(0);

			final TagMatch customer = tagMatch.getTagMatch(TagMatchDefinitions.CUSTOMER_ID);

			if (customer == null || customer.getTagValue().trim().length() == 0 || !(customer.getTagValue().equals("0") || checkNumberInput(customer))) {
			...
			if (contract != null && contract.getTagValue().trim().length() > 0) {
				if (checkNumberInput(contract)) {
			...
			final TagMatch mandate = tagMatch.getTagMatch(TagMatchDefinitions.SEPA_MANDATE_NUMBER);
			if (mandate != null && mandate.getTagValue().trim().length() > 0) {
				topLevel.add(mandate);
			}

			ValidationFormtypePanel_Sky.this.document.setTags(new ArrayList<TagMatch>());
			doc.setTags(tmList);

			Window activeWindow = KeyboardFocusManager.getCurrentKeyboardFocusManager().getActiveWindow();
			IClientAPI clientAPI = API.getClientAPI();
			clientAPI.getQuestionAPI().protocolLog(question, "ManualValidation:" + " DocumentID=" + doc.getNote(TagMatchDefinitions.DOCUMENT_ID) + ", CustomerID=" + customer.getTagValue() + ", ContractID=" + contract.getTagValue() + ", Formtype=" + formtype, OperatorLogRecord.ACTION_CHECKLIST);
		...
		private boolean checkNumberInput(TagMatch customer) {
			...
			if (textValue != null && textValue.length() > 0) {
				...
				request.addColumn(ValidationFormtypePanel_Sky.tagNames.get(customer.getIdentifier()), textValue, true, 0, mode);
				parameter.add(request);
				List<Object> result = (List<Object>) API.getClientAPI().getConnectionAPI().sendServerEvent(Actions.ACTION_FUZZY_SEARCH.name(), parameter);
	...
	public class SoftUnfocusTableAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (ValidationFormtypePanel_Sky.this.tbSearchResults.isFocusOwner()) {
				System.out.println("LastFocused:" + ValidationFormtypePanel_Sky.lastFocusComponent);
				ValidationFormtypePanel_Sky.lastFocusComponent.requestFocus();
				ValidationFormtypePanel_Sky.this.tbSearchResults.setModel(new DefaultTableModel());
			}
		}
	}
	...
	public class UnfocusTableAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (ValidationFormtypePanel_Sky.this.tbSearchResults.isFocusOwner()) {
				...
				// übertragen:
				valueChanged(new ListSelectionEvent(ValidationFormtypePanel_Sky.this.tbSearchResults, ValidationFormtypePanel_Sky.this.tbSearchResults.getSelectedRow(), ValidationFormtypePanel_Sky.this.tbSearchResults.getSelectedRow(), true));
				((DefaultTableModel) ValidationFormtypePanel_Sky.this.tbSearchResults.getModel()).setRowCount(0);
			}
		}
	}
	...
	public class NextPageAction extends AbstractAction {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			final int selectedpage = ValidationFormtypePanel_Sky.this.view.getDocumentViewer().getSelectedpage();
			if (selectedpage + 1 < ValidationFormtypePanel_Sky.this.view.getDocumentViewer().getDocument().getPageCount()) {
				ValidationFormtypePanel_Sky.this.requestFocus();
				SwingUtilities.invokeLater(new Runnable() {
					@Override
					public void run() {
						ValidationFormtypePanel_Sky.this.view.getDocumentViewer().selectPage(selectedpage + 1);
					}
				});
			}
		}
	...
	public ValidationFormtypePanel_Sky() {
		super();
		SkyLogger.getClientLogger().info("ValidationFormtypePanel1");
	}
	
	/**
	 * The default constructor.
	 */
	public ValidationFormtypePanel_Sky(JFrame parent) {
//		super(parent);
		SkyLogger.getClientLogger().info("ValidationFormtypePanel2");
	}
	 ...
	 * @param formpath
	 */
	public ValidationFormtypePanel_Sky(JFrame parent, Boolean tableView, String formpath) {
//		super(parent, tableView, formpath);
		SkyLogger.getClientLogger().info("ValidationFormtypePanel3");
	}
	...
	private void initDocumentView() {
		...
		readValidationData((String) documentForPage.getNote(TagMatchDefinitions.AUTOPROCESSING_FLAG));
		...
		this.gbc.ipady = 10;
		initButtonPanel();
		String formtype = (String) documentForPage.getNote(TagMatchDefinitions.EVAL_FORMTYPE);
		SkyLogger.getClientLogger().info("Formtype: " + formtype);
		initFormTypeBox();
		...
		initFuzzyTable();
		//#152379
		//if (formtype != null && formtype.equals(TagMatchDefinitions.SEPA_MANDATE)) {
	...
	private void initFormTypeBox() {
		...
		this.inputBox.getCbComboBox().setModel(new CCBoxModel<String>(this.validationData.get(TagMatchDefinitions.FORM_TYPE_CATEGORY)));
		((CCBox) this.inputBox.getCbComboBox()).setStrict(true);
	...
	private void initTagMatchPanel() {
		...
		int gridy = 0;
		SkyLogger.getMediatrixLogger().debug("saveAction: " + getView().getActionMap().get("save"));
		SkyLogger.getClientLogger().debug("saveAction: " + getView().getActionMap().get("save"));
		System.err.println("saveAction: " + getView().getActionMap().get("save"));
		System.out.println("saveAction: " + getView().getActionMap().get("save"));
		
		ValidationFormtypePanel_Sky.viewElements.clear();
		SkyLogger.getClientLogger().debug("saveAction: " + getView().getActionMap().get("save"));
		gbc.anchor = GridBagConstraints.EAST;
		gbc.weightx = 0.5;
		gbc.gridwidth = 1;
		for (TagMatch tm : tagmatches) {
			...
			if (ValidationFormtypePanel_Sky.firstColumnTags.contains(identifier)) {
				gbc.gridx = 0;
				gridy++;
				gbc.gridy = gridy;
			} else {
			...
			ValidationFormtypePanel_Sky.viewElements.put(identifier, tagView);
	...
	/**
	 * Setzt im aktuell selektierten Dokument den Formtype mit dem aktuellen
	 * Wert der ComboBox.
	 */
	@SuppressWarnings("unchecked")
	private void setFormtypeInDocument() {
		...
		SkyLogger.getClientLogger().debug("setFormType: " + formtype + ", selectedFormType: " + selectedFormType);
		System.out.println("setFormType: " + formtype + ", selectedFormType: " + selectedFormType);
		System.err.println("setFormType: " + formtype + ", selectedFormType: " + selectedFormType);
	...
	public boolean setFormType(String newValue, int page, int x1, int x2, int y1, int y2) {
		...
		SkyLogger.getMediatrixLogger().debug("setFormType: " + newValue);
		SkyLogger.getClientLogger().debug("setFormType: " + newValue);
		System.out.println("setFormType: " + newValue);
		System.err.println("setFormType: " + newValue);
		setFormtypeInDocument();
		return false;
	}

	// Read the selectable formtypes.
	private void readValidationData(String autoprocessing) {
		SkyLogger.getClientLogger().info(getClass().getName() + " readValidationData");
		SkyLogger.getClientLogger().info("Autoprocessing: " + autoprocessing);
		this.validationData = new TreeMap<String, Vector<String>>();
		Vector<String> formtypeKeywords = ClientOutboundRule.loadFormtypeKeywords(this.projektId);
		Enumeration<String> keywords = formtypeKeywords.elements();
		if (autoprocessing != null && autoprocessing.equals(TagMatchDefinitions.SEPA_MANDATE)) {
			while (keywords.hasMoreElements()) {
				String key = keywords.nextElement();
				if (key.equals(TagMatchDefinitions.SEPA_MANDATE)) {
					formtypeKeywords.remove(key);
				}
			}
		}
		this.validationData.put(TagMatchDefinitions.FORM_TYPE_CATEGORY, formtypeKeywords);
	}
	...
	public void keyTyped(KeyEvent arg0) {
		...
		System.out.println(logPrefix + ": enter");
	...
	protected void setTableModel(boolean fillTagMatches) {
		...
		if(dataModel.getRowCount()>10) {
			dataModel.setRowCount(10);
		}
		ValidationFormtypePanel_Sky.this.tbSearchResults.setModel(dataModel);
	...
	public void setValueFromExternalView(String newValue, int page, int x1, int x2, int y1, int y2) {
		...
		if (ValidationFormtypePanel_Sky.lastFocusComponent != null && ValidationFormtypePanel_Sky.lastFocusComponent instanceof JTextField) {
			if (ValidationFormtypePanel_Sky.lastFocusComponent.getName().equals("attribute3") && newValue.length() > 9) { //$NON-NLS-1$
				newValue = newValue.substring(0, 9);
			}
			((JTextField) ValidationFormtypePanel_Sky.lastFocusComponent).setText(newValue);
			TagMatch tagmatch = ((ITagMatchViewElement) ValidationFormtypePanel_Sky.lastFocusComponent.getParent()).getTagmatch();
			tagmatch.setPage(page + 1);
			tagmatch.setX1(x1);
			tagmatch.setX2(x2);
			tagmatch.setY1(y1);
			tagmatch.setY2(y2);
			if (!ValidationFormtypePanel_Sky.lastFocusComponent.getName().equals("attribute3")) { //$NON-NLS-1$
				ValidationFormtypePanel_Sky.lastFocusComponent.transferFocus();
			} else {
				ValidationFormtypePanel_Sky.lastFocusComponent.requestFocus();
			}
		} else if (ValidationFormtypePanel_Sky.lastFocusComponent != null && ValidationFormtypePanel_Sky.lastFocusComponent instanceof JComboBox) {
			((JComboBox) ValidationFormtypePanel_Sky.lastFocusComponent).setSelectedItem(newValue);
			ValidationFormtypePanel_Sky.lastFocusComponent.transferFocus();
		}
	}
	 ...
	 *
	 * @param parent The parent to add the button
	 * @param gbc    The layout
	 */
	...
	public static void setLastOp(Boolean lastOp) {
		ValidationFormtypePanel_Sky.lastOp = lastOp;
	}
}
