...
public class ReindexAction extends AbstractAction {
	...
	ReindexAction(String name, Icon icon) {
		super(name, icon);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			if (dialog == JOptionPane.OK_OPTION) {
				...
				String contractID = contractField.getText();
				...
				if (trying) {
					...
					SkyLogger.getClientLogger().debug("Starting ReindexAction with parameters:p:" + question.getProjectId() + " fid:" + question.getId() + " cid:" + customerNumber + " vid:" + contractID + " op:" + API.getClientAPI().getConnectionAPI().getCurrentOperatorShort().getId() + " ch:" + channel + " formtype" + formtype);
					...
					if (result == null || result.size() < 1) {
						trying = false;
						SkyLogger.getClientLogger().debug("Kunde nicht gefunden in FuzzyDB:p:" + question.getProjectId() + " fid:" + question.getId() + " cid:" + customerNumber + " vid:" + contractID + " op:" + API.getClientAPI().getConnectionAPI().getCurrentOperatorShort().getId() + " ch:" + channel + " formtype" + formtype);
						...
						if (firstres instanceof Boolean) {
							...
							SkyLogger.getClientLogger().warn("Problem with Reindex: " + question.getId() + " kunde:" + customerID + " Vertrag:" + contractID + " problem:" + problem);
						}
					...
					if (!trying) {
						...
						if (success) {
							...
							SkyLogger.getClientLogger().warn("Problem with Reindex: " + question.getId() + " kunde:" + customerID + " Vertrag:" + contractID);
							String problem = "";
							int i = 0;
							for (Object r : result) {
